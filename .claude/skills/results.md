...
- Criterion 9 (content-warning format/coverage): pass, with 2 unresolved low-confidence items —
...
- Criterion 10 (heat level correctness): pass, with 1 unresolved item — *The Matchmaker's List* (Sonya Lalli): no source found confirmed this as open- or closed-door; the run left off a heat-level line (defaulting to open-door), which is unverified either way.
